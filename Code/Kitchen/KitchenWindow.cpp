...
/// @brief this links the waiter to the window for the order hey brought in and
/// stops other orders from starting until this one has finished.
/// @param waiter the waiter that is being attached.
void KitchenWindow::attach(Waiter *waiter) {
  this->waitingWaiters = waiter;
  this->full = true;
}
/// @brief this finishes the cooking proces by opening up a new slot for a
/// waiter.
